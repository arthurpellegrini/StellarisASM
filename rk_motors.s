...
MOTEUR_INIT	
		...
		ldr	r10, [r12]
        ORR	r10, r10, #0x00100000  ;;bit 20 = PWM recoit clock: ON (p271) 
        str r10, [r12]
	...
	;*(int *) (0x400FE060)= *(int *)(0x400FE060)...;
  	...
  	;RCGC2 :  Enable port D GPIO(p291 ) car Moteur Droit sur port D 
		...
		ldr	r10, [r12] 		
        ORR	r10, r10, #0x08  ;; Enable port D GPIO 
        str r10, [r12]

	;MOT2 : RCGC2 :  Enable port H GPIO  (2eme moteurs)
		ldr r12, = SYSCTL_RCGC2
		ldr	r10, [r12] 
        ORR	r10, r10, #0x80  ;; Enable port H GPIO 
        str r10, [r12] 
	...
	;;il faut mettre 1 pour avoir PD0=PWM0 et PD1=PWM1
		ldr r12, = GPIOPCTL_D
		;ldr	r10, [r12] 	 ;;	*(int *)(0x40007000+0x0000052C)=1;
        ;ORR	r10, r10, #0x01 ;; Port D, pin 1 = PWM 
		mov	r10, #0x01  
        str r10, [r12]
	...
	;;donc PD0 = 1
		ldr r12, = GPIOAFSEL_D
		ldr	r10, [r12] 	  ;*(int *)(0x40007000+0x00000420)= *(int *)(0x40007000+0x00000420) | 0x00000001;
        ORR	r10, r10, #0x01 ;
        str r10, [r12]
	...
	;;Config Modes PWM0 + mode GenA + mode GenB
		ldr r12, = PWM0CTL
		mov	r10, #2		;Mode up-down-up-down, pas synchro
        str r10, [r12]	
		...
		mov	r10,	#0x0B0 	;0B0=10110000 => ACTCMPBD=00 (B down:rien), ACTCMPBU=00(B up rien)
		str r10, [r12]	;ACTCMPAD=10 (A down:pwmA low), ACTCMPAU=11 (A up:pwmA high) , ACTLOAD=00,ACTZERO=00  
		
		ldr r12, =PWM0GENB;en comptage croissant, qd comparateurB = compteur => sortie pwmA=1
		mov	r10,	#0x0B00	;en decomptage, qd comparateurB = compteur => sortie pwmB=0
		str r10, [r12]	
	...
	;;PWM1block produit PWM2 et PWM3 output
		...
		ldr r12, = PWM1CTL
		mov	r10, #2		;Mode up-down-up-down, pas synchro
        str r10, [r12]	;*(int *)(0x40028000+0x040)=2;
		...
		mov	r10,	#0x0B00	;en decomptage, qd comparateurB = compteur => sortie pwmB=0
		str r10, [r12]	;en comptage croissant, qd comparateurB = compteur => sortie pwmA=1
	;Config Compteur, comparateur A et comparateur B
  	;;#define PWM_PERIOD (ROM_SysCtlClockGet() / 16000),
	;;en mesure : SysCtlClockGet=0F42400h, /16=0x3E8, 
	;;on divise par 2 car moteur 6v sur alim 12v
		...
		mov	r10,	#VITESSE
		str	r10, [r12]  ;*(int *)(0x40028000+0x058)=0x01C2;
		
		ldr	r12, =PWM1CMPB ;PWM0CMPB recoit meme valeur. (CMPA depend du rapport cyclique)
		mov	r10,	#0x1F4	; *(int *)(0x40028000+0x05C)=0x1F4; 
		str	r10,	[r12]
		
	;Control PWM : active PWM Generator 0 (p1167): Enable+up/down + Enable counter debug mod
		ldr	r12, =PWM1CTL 
		ldr	r10, [r12]	;*(int *) (0x40028000+0x40)= *(int *)(0x40028000+0x40) | 0x07;
		ORR	r10,	r10,	#0x07
		str	r10,	[r12]		
...
MOTEUR_GAUCHE_ARRIERE
		;Inverse Direction (GPIO_D1)
		ldr	r12, =(GPIODATA_H+(GPIO_1<<2)) 
		mov	r10, #2 ; contraire du moteur Droit
		str	r10,[r12]
		BX	LR		
